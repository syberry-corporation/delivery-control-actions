...
The gates read files and registry referrers; three actions produce them, with the same scripts as the
GitLab components:

| Action | Writes | Read by |
|---|---|---|
| `security-scan` | `supervisory/security-scan.json`, `supervisory/dependency-inventory.json` | `supervise-pr-gate` (SD-2.4.3, SCM-3.1) |
| `build-image` | the image under a version tag, its SBOM and SLSA provenance as OCI referrers | `supervise-build-gate` (SD-1.1 … SD-1.5) |
| `publish-static-artifact` | `<version>.zip`, its SBOM and provenance in the artifact bucket, and the metadata file | `supervise-build-gate-artifact`, `supervise-deploy-gate` |

```yaml
# PR: scan, then hand the two files to the PR gate (upload `supervisory/`, download it back to `supervisory`)
- uses: syberry-corporation/delivery-control-actions/security-scan@1.4.29
  with:
...
- id: image
  uses: syberry-corporation/delivery-control-actions/build-image@1.4.29
  with:
...
# -> steps.image.outputs.digest / .image (repository@digest): deploy by it

# Static build: after `yarn build`, with credentials that can write the artifact bucket
- id: artifact
  uses: syberry-corporation/delivery-control-actions/publish-static-artifact@1.4.29
  with:
    artifact_base_uri: s3://${{ vars.ARTIFACT_BUCKET }}/frontend
    version: ${{ steps.v.outputs.version }}
    build_folder: build
# -> steps.artifact.outputs.metadata_file: pass it to supervise-build-gate-artifact (metadata_file) and to
#    the post-deploy gate (artifact_metadata_file)
```

`security-scan` fails the step on unwaived critical/high findings (`fail_on_findings: false` to report
only); the evidence is written either way. On Node older than 16.9 set `install_command` (the default
enables corepack, which those versions do not ship).
...
jobs:
  supervise:
    ...
    steps:
      ...
      - uses: syberry-corporation/delivery-control-actions/supervise-pr-gate@1.4.29
        with:
...
- uses: syberry-corporation/delivery-control-actions/supervise-build-gate@1.4.29
  with:
    supervisory_eval_url: ${{ vars.SUPERVISORY_EVAL_URL }}
    supervisory_project: ${{ vars.SUPERVISORY_PROJECT }}
    component: backend
...
```yaml
- uses: syberry-corporation/delivery-control-actions/supervise-build-gate-artifact@1.4.29
  with:
...
A build that publishes **no artifact at all** (a site synced straight from its build folder) still runs
`supervise-build-gate-artifact`, with `metadata_file` left out. The gate then submits exactly that — no
artifact, no SBOM, no provenance — and the build controls fail on it: the record shows the build and why it
does not hold, instead of showing nothing.
...
deploy:
  ...
  permissions:
    ...
    id-token: write
  steps:
    - uses: syberry-corporation/delivery-control-actions/supervise-deploy-gate@1.4.29
      with:
        phase: pre
        supervisory_eval_url: ${{ vars.SUPERVISORY_EVAL_URL }}
        supervisory_project: ${{ vars.SUPERVISORY_PROJECT }}
